...
import { Trash2, Pencil, Check, X, ArrowRightLeft } from "lucide-react";
import { getFermes, getTransfertsStock, creerTransfertStock, modifierTransfertStock, supprimerTransfertStock } from "../api/client";
...
export default function Transferts() {
  ...
  useEffect(() => {
    getFermes().then((data) => {
      setFermes(data);
      setForm((f) => (f.ferme_source ? f : { ...f, ferme_source: data[0]?.id ?? "" }));
    });
  }, []);
